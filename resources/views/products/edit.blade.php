                    ...
                    <div class="panel-body">
                        <form class="form-horizontal" role="form" method="POST" action="{{ url('/product/'.$product->id) }}">
                            ...
                            <div class="form-group">
                                <label for="sel1" class="col-md-4 control-label">Select Categories</label>
                                <div class="col-md-6">
                                    <select name="categories[]" multiple class="form-control" id="sel1">
                                        @foreach($categories as $category)
                                            <option {{count($product->categories->where('id',$category->id))?"selected":""}} value={{$category->id}}>{{$category->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
